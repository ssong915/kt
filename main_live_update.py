...
def train(args):
    ...
    os.makedirs(f"logs/{args.aggregator}", exist_ok=True)
    os.makedirs(f"logs/{args.aggregator}/{args.dim_vertex}/", exist_ok=True)
    os.makedirs(f"logs/{args.aggregator}_{args.neg_mode}/{args.dim_vertex}/", exist_ok=True)
    if args.time_split_type == 'sec': 
        f_log = open(f"logs/{args.aggregator}_{args.neg_mode}/{args.dim_vertex}/{DATA}_{args.time_split_type}_{args.freq_size}_{args.model}_train.log", "w")
    elif args.time_split_type == 'num':
        f_log = open(f"logs/{args.aggregator}/{args.dim_vertex}/{DATA}_{args.time_split_type}_{args.batch_size}_{args.model}_train.log", "w")
         
    f_log.write(f"args: {args}\n") 
    if args.time_split_type == 'sec': 
        print(f"💖 logs/{ args.aggregator}_{args.neg_mode}/{DATA}_{args.dim_edge}_{args.freq_size}_{args.model}_train.log")
    if args.time_split_type == 'num': 
        print(f"💖 logs/{ args.aggregator}_{args.neg_mode}/{DATA}_{args.dim_edge}_{args.freq_size}_{args.model}_train.log")
    roc_list = []
    ap_list = []   
    ...
    for i in range(args.exp_num): # number of splits (default: 5)
        ...
        if args.eval_mode == 'train_fixed_split' :
            avg_roc, avg_ap, auc_roc, ap = train_fixed_split.train(args, st_model, snapshot_data, decoder, optimizer, scheduler, f_log, i)
        else :
            avg_roc, avg_ap, auc_roc, ap = train_live_update.train(args, st_model, snapshot_data, decoder, optimizer, scheduler, f_log, i)
    ...
    print('[ LAST SNAPSHOT ]')
    print('AUROC\t AP\t ')
    print(f'{auc_roc:.4f}\t{ap:.4f}')
    print('[ AVG SNAPSHOT ]')
    print('AUROC\t AP\t ')
    print(f'{avg_roc:.4f}\t{avg_ap:.4f}')
    
    f_log.write(f'{final_roc:.4f}\t{final_ap:.4f}')
    f_log.write(f'{std_roc:.4f}\t{std_ap:.4f}')
