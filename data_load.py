...
def gen_data(args, snapshot_data):
    ...
    args.ne = ne
    data_dict['e_feat'] = torch.rand(ne, args.dim_edge).to(device)
    ...
    data_dict['HGNN_G'] = generate_G_from_H(data_dict['h_incidence'])
    ...
    data_dict['e_reg_weight'] = torch.Tensor(e_reg_weight).unsqueeze(-1).to(device)
    data_dict['v_reg_sum'] = torch.Tensor(v_reg_sum).unsqueeze(-1).to(device)
    data_dict['v_reg_weight'] = torch.Tensor(v_reg_weight).unsqueeze(-1).to(device)
    data_dict['e_reg_sum'] = torch.Tensor(e_reg_sum).unsqueeze(-1).to(device)
...
def load_snapshot(args, DATA):
    ...
    # 1. get feature and edge index        
    r_data = pd.read_csv(f'/home/dake/workspace/HNHN,HGNN/data/{DATA}/new_hyper_{DATA}.csv')
